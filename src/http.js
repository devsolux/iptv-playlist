...
const hasCancelToken = !!(CancelToken && typeof CancelToken.source === 'function')
...
curlirize(streamClient, result => {
    try {
        if (result && result.config && result.config.curlirize && result.command) {
            console.log(`CURL: "${result.command}"`)
        }
    } catch (_) {
        // no-op
    }
...
function parseError(err, config, logger) {
    ...
    } else if (typeof err.code === 'string' && err.code.startsWith('HPE')) {
