...
const parsedTimeout = Number.isFinite(parseInt(options.timeout, 10)) ? parseInt(options.timeout, 10) : 60000
const parsedParallel = Number.isFinite(parseInt(options.parallel, 10)) ? parseInt(options.parallel, 10) : 1
...
const onlineFile = path.join(outputDir, 'online.m3u')
const offlineFile = path.join(outputDir, 'offline.m3u')
const duplicatesFile = path.join(outputDir, 'duplicates.m3u')
...
async function init() {
    try {
        ...
        stats.online = checked.items.filter(item => item.status && item.status.ok).length
        stats.offline = checked.items.filter(item => item.status && !item.status.ok && item.status.code !== 'DUPLICATE').length
        stats.duplicates = checked.items.filter(item => item.status && !item.status.ok && item.status.code === 'DUPLICATE').length
...
function writeToFile(filePath, item, message = null) {
